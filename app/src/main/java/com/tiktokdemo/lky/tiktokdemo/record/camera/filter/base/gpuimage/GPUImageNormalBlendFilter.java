...
public class GPUImageNormalBlendFilter extends GPUImageTwoInputFilter {
    public static String NORMAL_BLEND_FRAGMENT_SHADER = "varying highp vec2 textureCoordinate;\n" +
    ...
    public GPUImageNormalBlendFilter(int position) {
        super(getFragmentShader(position),position);
    }

    public static String getFragmentShader(int position){
        return "varying highp vec2 textureCoordinate;\n" +
                " varying highp vec2 textureCoordinate" + position + ";\n" +
                " \n" +
                " uniform sampler2D inputImageTexture;\n" +
                " uniform sampler2D inputImageTexture" + position + ";\n" +
                " \n" +
                " void main()\n" +
                " {\n" +
                "     lowp vec4 c2 = texture2D(inputImageTexture, textureCoordinate);\n" +
                "\t lowp vec4 c1 = texture2D(inputImageTexture" + position + ", textureCoordinate" + position + ");\n" +
                "     \n" +
                "     lowp vec4 outputColor;\n" +
                "     \n" +
                "     outputColor.r = c1.r + c2.r * c2.a * (1.0 - c1.a);\n" +
